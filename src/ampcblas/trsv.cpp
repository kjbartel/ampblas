...
{
    AMPBLAS_CHECKED_CALL( ampblas::trsv(order, Uplo, TransA, Diag, N, A, lda, X, incX) );
}
...
{
    AMPBLAS_CHECKED_CALL( ampblas::trsv(order, Uplo, TransA, Diag, N, ampblas_cast(A), lda, ampblas_cast(X), incX) );
}
